...
for(k in seq(from=1, to=length(cancer1), by=1))
{
  ...
  genes <- scan(file = '/Users/macbook/Desktop/pancangenes.csv', what = 'character', sep = ',')
...
{  g=v/2
  ...
  ggsave('/Users/macbook/Desktop/KMplots/'+cancer+'pangenes_voting.jpg', plot = print(gg),height = 5 ,width = 7, dpi = 1000)
  
  
  {write.table(cbind(cancer,first[2],first[5],a$waldtest[3],a$logtest[3],a$concordance[1],a$conf.int[3],a$conf.int[4],min(PI),max(PI),g),
